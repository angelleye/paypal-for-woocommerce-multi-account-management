...
class Paypal_For_Woocommerce_Multi_Account_Management_List_Data extends WP_List_Table {
            ...
            case 'status':
                ...
                if ($status == 'on') {
                    ...
                    if (get_post_status($item['ID']) === 'draft') {
                        return __('Invitation Not Sent', 'paypal-for-woocommerce-multi-account-management');
                    } elseif (empty($board_status)) {
    ...
    protected function get_views() {
        ...
        if ($count_posts) {
            $published_posts = $count_posts->publish + $count_posts->draft;
        ...
        $all_url = remove_query_arg('filter_entity', admin_url('admin.php?page=wc-settings&tab=multi_account_management'));
        $views['all'] = "<a href='{$all_url}' {$class}>All ($published_posts)</a>";

        // Trash link
        $class = ($current == 'trashed' ? ' class="current"' : '');
        $trashed_url = add_query_arg('filter_entity', 'trashed', admin_url('admin.php?page=wc-settings&tab=multi_account_management'));
        $views['trashed'] = "<a href='{$trashed_url}' {$class}>Trashed ($trashed_posts)</a>";

        // Need Email link
        $class = ($current == 'need_email' ? ' class="current"' : '');
        $need_email_url = add_query_arg('filter_entity', 'need_email', admin_url('admin.php?page=wc-settings&tab=multi_account_management'));
        $views['need_email'] = "<a href='{$need_email_url}' {$class}>Need Email ($need_email)</a>";
